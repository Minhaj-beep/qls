...
import {
  ...
  const SCView = ({navigation}) => {
    ...
    const [demoTimerId, setDemoTimerId] = useState(null);
    ...
    useEffect(()=>{ //Starting the timer and ending it if focus changes
      if(isFocused && !type) {
    ...
    },[isFocused, type, liveClassList])
  
    useEffect(()=>{ // ending the timer while leaving the compenent
      if(demoTimerId !== null){
        return ()=> {
          console.log('_______________________UNMOUNTED__________________________')
          endTimer()
    ...
    const startTimer = () => { // start timer for demo class by course code and finding live class for today if Live course
      let sec = 0
      ...
      setDemoTimerId(check)
    }

    const endTimer = () => { // closing the timer
