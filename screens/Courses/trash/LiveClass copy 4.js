...
const LiveClass = ({navigation}) => {
  ...
  const [messageList, setMessageList] = useState([{
    username: 'Minhaj', message: 'Hello', createdAt: 1672329948874
  }])
  ...
  // This is the current ongoing messages response.message response.username response.createdAt
  socket.on("message", (response) => {
    ...
    console.log('Is this the responce :' + response.message)
  })

  useEffect(()=>{
    // messageList.map((i)=>{console.log(i)})
  ...
  const RenderChat = () => {
    ...
    return messageList.map((data,index)=>{
        ...
        return (
                ...
                //     <HStack space={2} >
                //     <Text style={{fontSize: 12,color: '#000000',fontWeight: 'bold'}}>{data.username}</Text>
                //     <Text style={{fontSize: 9,color: '#8C8C8C'}}>{data.createdAt}</Text>
                //     </HStack>
