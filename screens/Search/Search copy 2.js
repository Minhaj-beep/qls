...
import GetSearchedCourses from "../Functions/API/GetSearchedCourses";
...
const IndependentAssessment = () => {
    ...
    const getAllActiveAssessments = async () => {
        try {
            let response = await GetAllCourses(email)
            if (response.status === 200){
    ...
    const searchAsssessment = async() => {
        try {
            let response = await GetSearchedCourses(email, query)
            if (response.status === 200){
                setAllData(response.data)
                console.log('Get All Courses: ',response.data)
            } else {
                console.log('getAllActivatedAssessment', response.message)
            }
        } catch (e) {
